...
fires_matrix_path = strcat(mouse,'/DeltaDetection/fires/fires_matrix.mat'); 
...
%% Load Detections and create offline detection
load(fires_actual_time_path);
Detection{1} = {fires_actual_time*1e-4}; clear fires_actual_time
load(fires_matrix_path); 
Detection{2} = {fires*1e-4}; clear fires
load(detection_matrix_path); 
Detection{3} = {detections*1e-4}; clear detections;
disp('Computing offline detection ...');
offlinedetection = CreateDeltaWavesOffline (mouse,detection_matrix_path);
disp('done');
Detection{4} = offlinedetection;
load(sleepstagepath);

%% Detections Selection 

prompt = 'Select online Detection to compare \n 1:fires_actual_time \n 2:fires_matrix \n 3:detection_matrix \n';
first = input(prompt);
...
if isstruct(Detection{1,4})
    Detection_2 = cell2mat(struct2cell(Detection{1,4}));
elseif iscell(Detection{1,4})
    Detection_2 = cell2mat(Detection{1,4});
else 
    Detection_2 = Detection{1,4};
end
...
awnser = input(prompt);
if awnser == 1
    prompt = 'Which Detection (1 = online/2 = offline) \n';
    num = input(prompt);
...
if mode == 1 
    ...
    k = figure;
    set(gcf,'Color',[1 1 1]);
    str0 = strcat(num2str(size(online_durations_tot,1)),'delta waves detected online');
