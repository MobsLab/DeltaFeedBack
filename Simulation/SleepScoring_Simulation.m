...
%This UI simulates Intan live acquisition. User can load LFP signals and 
%the parameter mouse file matlab window when clicking on load button.
%(See load function)

%It performs Sleep Scoring, Delta Waves detection.

%To immitate read continuously live acquisition, LFP signals are read DataBlock per DataBlock at 312.5 Hz.

%Do not forget to click Preprocessing button for downsampling, filtering,
%thresholds and prefactors computations. 

%A timer object calls Update_Display Callback function, in which other
%Tool_Functions (see Functions folder) are called to update data, perform treatments
%and display results.

%Different Diplay options are proposed to user from the interface with
%RadioButton groups. 

%Detection matrix is saved at the end in a path selected by the user from a
%matlab window. 


function SleepScoring_Simulation

addpath(genpath('/home/mobsspectre/Dropbox/Mobs_member/AdrienBaptisteProject/Simulation'));
...
AllData.T0 = [];                                                            %Starting time (S) for Time Selection
AllData.T1 = [];                                                            %Endinf time (s) for Time Selection 

AllData.delta_points_counter = 0;                                           %counting current delta wave duration (in number of points)
AllData.Delta_Detection = [];                                               %Delta Detection Matrix [Start Stop]
...
AllData.min_duration = 50;                                                  %Min duration (in ms) for Delta Wave (condition)
AllData.max_duration = 150;                                                 %Max duration (in ms) dor Delta Wave (condition)       

AllData.ratio_treshold_temp = [];                                           %Power ratio theta/delta threshold for Sleep Scoring
AllData.gamma_treshold_temp= [];                                            %Gamma threshold for Sleep Scoring
AllData.delta_treshold_temp= [];                                            %Delta threshold for Delta Detection 

AllData.sup_prefactor = 1;                                                  %PFC sup multiplicative prefactor 
AllData.deep_prefactor = 1;                                                 %PFC deep multiplicative prefactor 

AllData.filter_order = 4;                                                   %filter for (PFCdeep - PFCsup signal)
AllData.cutoff_freq = 8;                                            

AllData.S.S1.gamma_power = [];                                              %Powers for sleepscoring 
AllData.S.S2.theta_power = [];
AllData.S.S2.delta_power = [];
AllData.S.S2.ratio_power = [];

AllData.S.S3.PFCdeep_DeltaMean = zeros(251,1);                              %Mean PFCsup and PFCdeep computed during Delta detections
AllData.S.S3.PFCsup_DeltaMean = zeros(251,1);

AllData.sleep_matrix = [];                             
AllData.AllResults = [];                                                    %Matrix containing TimeStamps and corresponding Sleep Stage

AllData.Wake_counter = 0;                                                   %Counters for Statistics 
...
AllData.PreProcessed = 0;                                                   %Indicators (1=Done; 0=Not done)
...
Interface.Detected = 0;                                                     %Indicator (=1 when Delta Wave is Detected)
Interface.counter_sinceDetection = 0;                                       %counting elapsed time since last detection, important to wait sufficient time before update snapshot     
Interface.Snapshot = [];
Interface.Detectionline1 = [];
Interface.Detectionline2 = [];
Interface.ActualDeltaNum = [];
Interface.Navigating_Since = 0;                                             %counting elapsed time since navigation mode is activated

Interface.offset_sup = 1;                                                   %offsets for PFCsup and PFCdeep plots 
...
set(Interface.phase_space_plot_lines(2),'LineStyle','None','Marker','.','MarkerSize',10,'Color',[0 0 0.8]);
set(Interface.phase_space_plot_lines(3),'LineStyle','None','Marker','.','MarkerSize',10,'Color',[0 0.8 0]);
